    ...
    class _TrainingModule(nn.Module):
        def __init__(self, main_model):
            ...
            self.target_index = list(
                self.main_model.similarity_layer.fidelity_layer.output_keys
            ).index(tuple(self.main_model.similarity_layer.fidelity_layer.input_state))
        ...
        for epoch in range(num_epochs):
            ...
            if return_data:
                with torch.no_grad():
                    ...
                    for class_index in range(self.num_classes):
                        # Training states
                        classical_data = self.classical_encoder(
                            X_train_splits[class_index]
                        )
                        states = assign_params(
                            self.quantum_embedding_layer, classical_data
                        )
